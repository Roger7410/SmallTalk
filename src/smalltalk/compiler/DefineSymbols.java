...
public class DefineSymbols extends SmalltalkBaseListener {
	...
	public void enterClassDef(@NotNull SmalltalkParser.ClassDefContext ctx) {
        ...
        List<String> sFieldsNames = new ArrayList<>();
		if ( ctx.ID(1)!=null ) {
			superClassName = ctx.ID(1).getText();
            // for inherit
            if (currentScope.resolve(superClassName) != null) {
                STClass sClass = (STClass) currentScope.resolve(superClassName);
                if (sClass.getFields() != null) {
                    List<FieldSymbol> fields = new ArrayList<>(sClass.getFields());
                    for (Symbol f: fields){
                        sFieldsNames.add(f.getName());
                    }
                }
            }
		}
        ...
        cl.setEnclosingScope(currentScope);//STMETA  mark
	...
	public void enterSmalltalkMethodBlock(@NotNull SmalltalkParser.SmalltalkMethodBlockContext ctx) {
		...
		STMethod m = new STMethod(ctx.selector, methodNode);
...
//										   methodNode);
        STMethod m = new STPrimitiveMethod(ctx.selector, methodNode, primitiveName);
		currentScope.define(m);
		compiler.defineArguments(m, ctx.args);
		methodNode.scope = m;
		currentMethod = m;
...
//				STVariable v = new STVariable(varName);
//				currentScope.define(v);
//			compiler.defineLocals(currentScope, vars);
//			}
            // move them to defineLocals to make it more clear
            compiler.defineLocals(currentScope, vars);
		}
	}
...
//			}
	...
	public void enterBlock(@NotNull SmalltalkParser.BlockContext ctx) {
		...
		STBlock blk = new STBlock(currentMethod, ctx);
